...
from CustomDataset import CustomDataset, HMDataset
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (strain_features, curvature_target, dir_target, force_target, force_loc_target, twist_target, pos_xyz_target) in enumerate(train_loader):
        ...
        force_loc_pred_index = torch.argmax(force_loc_pred, dim=1)
        force_loc_target_index = torch.argmax(force_loc_target, dim=1)
        force_loc_norm_err = force_loc_pred_index - force_loc_target_index
        force_loc_err = torch.abs(force_loc_norm_err)
...
with torch.no_grad():
    for batch_idx, (strain_features, curvature_target, dir_target, force_target, force_loc_target, twist_target,
        ...
        force_target_dn_array = force_target_array * (dataset.force.max() - dataset.force.min()) + dataset.force.min()

        force_loc_target_dn_array = torch.argmax(force_loc_target, dim=1, keepdims=True).cpu().numpy()
        force_target_dn_array_out = np.zeros((force_target_dn_array.shape[0], 36))
        row_index = np.arange(force_target_array.shape[0])
        column_index = force_loc_target_dn_array[:, 0]
        column_index = np.clip(column_index, 0, 35)
        force_target_dn_array_out[row_index, column_index] = force_target_dn_array[row_index, 0]
        ...
        raw_data = np.concatenate((strain_features_dn_array, curvature_target_dn_array, dir_target_array, force_target_dn_array_out, force_loc_target_dn_array, twist_target_dn_array, pos_xyz_target_dn_array), axis=1)
        ...
        force_loc_pred_dn_arr = torch.argmax(force_loc_pred, dim=1, keepdims=True).cpu().numpy()
        force_pred_dn_array_out = np.zeros((force_pred_dn_arr.shape[0], 36))
        row_index = np.arange(force_pred_arr.shape[0])
        column_index = force_loc_pred_dn_arr[:, 0]
        column_index = np.clip(column_index, 0, 35)
        force_pred_dn_array_out[row_index, column_index] = force_pred_dn_arr[row_index, 0]
...
print('Overall tip error: \n', np.mean(merged_results_array[:, -4:-2], axis=0), np.max(merged_results_array[:, -4:-2], axis=0))
print('Force error: \n', np.mean(merged_results_array[:, -2], axis=0), np.max(merged_results_array[:, -2], axis=0))
print('Force location error: \n', np.mean(merged_results_array[:, -1], axis=0), np.max(merged_results_array[:, -1], axis=0))
